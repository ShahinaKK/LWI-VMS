...
def main():
    ...
    parser.add_argument("--shuffle_pretrain", action="store_true", help="ssl shuffling if true")
    parser.add_argument("--pretrain_ckpt", default="./model_200.pt", type=str, help="pretrained model weights")
    parser.add_argument("--mask_patch_size", default=0.4, type=int, help="size of masking patch")
    parser.add_argument("--mask_ratio", default=0.4, type=float, help="portion of image to be masked")

    args = parser.parse_args()
    do_shuffle =args.shuffle_pretrain
    
    if do_shuffle:
        print("Initialization step")
    else:
        print("Training or validation")

    task = args.task
    fold = args.fold
    ckpt =args.pretrain_ckpt
    print("given ckpt: ", ckpt)
    mask_ratio =args.mask_ratio
    print("given mask ratio: ", mask_ratio)
    mask_patch_size =args.mask_patch_size
    print("given mask patch size: ", mask_patch_size)
    ...
    trainer = trainer_class(plans_file, fold, output_folder=output_folder_name, dataset_directory=dataset_directory,
                            ...
                            deterministic=deterministic, fp16=run_mixed_precision,do_shuffle=args.shuffle_pretrain, ckpt=ckpt, val=validation_only, mask_ratio=mask_ratio,mask_patch_size=mask_patch_size)
