...
class UNETR_PP(SegmentationNetwork):
    ...
    def forward(self, x_in):
        ...
        if self.do_shuffle:
          ...
          x_t1=rearrange(enc1, 'b e w h d -> b (w h d) e', w=32, h=48, d=48)
          x_t1 = rearrange(x_t1, 'b (w h d) e -> b d (w h) e', d=4 , h=48)
          x_tm1 = x_t1.mean(2)
          t1 = self.temp_head1(x_tm1)
          #print(t1.shape)
          
          #2:
          x_t2 = rearrange(enc2, 'b e w h d -> b (w h d) e',w=16, h=24, d=24)
          x_t2 = rearrange(x_t2, 'b (w h d) e -> b d (w h) e', d=4,h=24)
          x_tm2 = x_t2.mean(2)
          ...
          #3:
          x_t3 = rearrange(enc3, 'b e w h d -> b (w h d) e', w=8, h=12, d=12)
          x_t3 = rearrange(x_t3, 'b (w h d) e -> b d (w h) e',d=4, h=12)
          x_tm3 = x_t3.mean(2)
          t3 = self.temp_head3(x_tm3)
          #print(t3.shape)
  
          #4: 
          x_t4 = rearrange(enc4, 'b (w h d) e -> b d (w h) e', d=4, h=6, w=6)
          x_tm4 = x_t4.mean(2)
