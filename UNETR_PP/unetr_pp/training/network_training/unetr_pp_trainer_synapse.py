...
def patchify(imgs, patch_size):
    """
    imgs: (N, 1, H, W,D)#[2,1,64,128,128]
    x: (N, L, patch_size**3 *1)
    """
    p = patch_size #4
    assert imgs.shape[3] == imgs.shape[4] and imgs.shape[2] % p == 0 and imgs.shape[3] % p == 0

    w = d= imgs.shape[3] // p #32
    h=imgs.shape[2]//p #16
    x = imgs.reshape(shape=(imgs.shape[0], 1, h, p, w, p,d,p)) #[2, 1, 16, 4, 32, 4, 32, 4]
    x = torch.einsum('nchpwqdr->nhwdpqrc', x) #[2, 16, 32, 32, 4, 4, 4, 1]
    x = x.reshape(shape=(imgs.shape[0], h * w * d, p ** 3 * 1)) #[2, 16384, 64]
    return x
...
def unpatchify(x, patch_size):
    ...
    #h=math.ceil(x.shape[1]/16)
    s= depth/patch_size
    w = d = math.ceil((x.shape[1]/s) ** (1. / 2.))
    h= math.ceil(x.shape[1]/(w*d))
...
def mask(img, mask_ratio=0.75, patch_size=16):
    """
    ...
    N, L, D = patches.shape #[2, 16384, 64]
    len_mask = int(L * (mask_ratio)) #*16384 * 0.75=12288
    noise = torch.rand(N, L)  # noise in [0, 1]   #[2,16384]
...
def shuffle(img):
    #img : [2,1,64,128,128]
    inputs = img.transpose(1,2).contiguous() #[2,64,1,128,128]
    inputs_b = rearrange(inputs, 'b (n d) c h w -> b n d c h w', n=4) #[2, 4, 16, 1, 128, 128] 
    B, N, D, C, H, W = inputs_b.shape
    indices = np.random.permutation(N)
    shuffled_inputs = inputs_b[:, indices] #[2, 4, 16, 1, 128, 128]shuffled
    inputs_sh = rearrange(shuffled_inputs, 'b n d c h w -> b (n d) c h w', n=4) #[2,64, 1, 128, 128]shuffled
    inputs_final = inputs_sh.transpose(1, 2).contiguous() #[2, 1, 64, 128, 128]shuffled

    return indices,inputs_final
...
class unetr_pp_trainer_synapse(Trainer_synapse):
    ...
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
                 unpack_data=True, deterministic=True, fp16=False,do_shuffle=True,ckpt=None,val=False,mask_ratio=0.4,mask_patch_size=4):
        super().__init__(plans_file, fold, output_folder, dataset_directory, batch_dice, stage, unpack_data,
                         deterministic, fp16)
        self.max_num_epochs = 2 if do_shuffle else 2
        self.initial_lr = 1e-4 if do_shuffle else 1e-2
        ...
        print("shuffling: ",self.do_shuffle)
        self.ckpt =ckpt
        print("Load checkpoint: ",self.ckpt)
        self.val =val
        print("validate trained model") if val == True else print ("Not validation")
    ...
    def initialize(self, training=True, force_load_plans=False):
        ...
        if not self.was_initialized:
            ...
            self.do_shuffle =self.do_shuffle
            self.ckpt =self.ckpt
            self.val =self.val
            print("Load checkpoint_in: ",self.ckpt)
            if self.deep_supervision:
                ...
                self.multi_loss_rec=MultipleOutputLoss2(self.loss_rec, self.ds_loss_weights)
                print("####################ds_weights#################",self.ds_loss_weights)
                ################# END ###################
    ...
    def initialize_network(self):
        ...
        self.network = UNETR_PP(in_channels=self.input_channels,
                             out_channels=self.num_classes if (self.val ==True) else self.input_channels,
        ...
        if self.do_shuffle == False and self.val ==False:
               
        
          #ckpt ="/home/shahina.kunhimon/PycharmProjects/unetr_plus_plus/Ours_pretrain_output_synapse/unetr_pp/3d_fullres/Task002_Synapse/unetr_pp_trainer_synapse__unetr_pp_Plansv2.1/fold_0/model_ep_200.model"
          ckpt = self.ckpt
          checkpoint = torch.load(ckpt, map_location='cpu')
          self.network.load_state_dict(checkpoint['state_dict'])
          print('I am using the pre_train weights from {}!!'.format(ckpt))
          self.print_to_log_file('I am using the pre_train weights from {}!!'.format(ckpt))
                             
          self.network.out1 = UnetOutBlock(spatial_dims=3, in_channels=16, out_channels=14)
            
          self.network.out2 = UnetOutBlock(spatial_dims=3, in_channels=32, out_channels=14)
          self.network.out3 = UnetOutBlock(spatial_dims=3, in_channels=64, out_channels=14)
          print("*********training for segmentation*********")
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
        ...
        self.loss_rec= torch.nn.MSELoss()
        self.multi_loss_rec=MultipleOutputLoss2(self.loss_rec, self.ds_loss_weights)
        print("3333",self.ds_loss_weights)
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        to_shuffle =self.do_shuffle

        data = maybe_to_torch(data)
        
        if to_shuffle == True:
            indices,sh_data = shuffle(data)
            mask_sh_data = mask(sh_data,mask_ratio = self.mask_ratio,patch_size = self.mask_patch_size)
            data = mask_sh_data
            target=sh_data
        else:
            target = data_dict['target']
        ...
        if self.fp16:
            with autocast():
                if to_shuffle:
                  output, t1,t2,t3,t4 = self.network(data)
                  del data

                  B, T = t1.shape[:2]
                  t_label = torch.LongTensor(list(indices)).unsqueeze(0).repeat(B, 1).cuda()
                  l_rec= torch.nn.MSELoss()(output[0],target)
                  
                  l_cls1 = torch.nn.CrossEntropyLoss()(t1.view(B*T,-1), t_label.view(-1))
                  l_cls2 = torch.nn.CrossEntropyLoss()(t2.view(B*T,-1), t_label.view(-1))
                  l_cls3 = torch.nn.CrossEntropyLoss()(t3.view(B*T,-1), t_label.view(-1))
                  l_cls4 = torch.nn.CrossEntropyLoss()(t4.view(B*T,-1), t_label.view(-1))
                  
                  l_cls =l_cls1 + l_cls2 + l_cls3 + l_cls4
                  l=l_rec + l_cls
          ...
          if to_shuffle:
                  ...
                  t_label = torch.LongTensor(list(indices)).unsqueeze(0).repeat(B, 1).cuda()
                  l_rec= torch.nn.MSELoss()(output[0], target)
                  #_rec= self.multi_loss_rec(output,target)
