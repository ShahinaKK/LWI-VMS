...
def shuffle(img):
    inputs = img.transpose(1,2).contiguous()
    inputs_b = rearrange(inputs, 'b (n d) c h w -> b n d c h w', n=4) 
    B, N, D, C, H, W = inputs_b.shape
    indices = np.random.permutation(N)
    shuffled_inputs = inputs_b[:, indices] 
    inputs_sh = rearrange(shuffled_inputs, 'b n d c h w -> b (n d) c h w', n=4) 
...
class unetr_pp_trainer_lung(Trainer_lung):
    ...
    def initialize_network(self):
        ...
        if self.do_shuffle == False and self.val ==False:
          ...
          self.network.out1 = UnetOutBlock(spatial_dims=3, in_channels=16, out_channels=2)
            
          self.network.out2 = UnetOutBlock(spatial_dims=3, in_channels=32, out_channels=2)
          self.network.out3 = UnetOutBlock(spatial_dims=3, in_channels=64, out_channels=2)
